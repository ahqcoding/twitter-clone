...
export default function InputBox({ onAddPost }) {
  const formik = useFormik({
    initialValues: {
      content: "",
    },
    onSubmit: (values) => {
      onAddPost(values.content);
    },
  });
    ...
    <div className="px-5 py-3 border-lighter flex">
      <div className="flex-none">
        <img
          ...
          className="flex-none w-12 h-12 rounded-full border-lighter hover:bg-lightest"
        />
      </div>
      <form className="w-full px-4 relative" onSubmit={formik.handleSubmit}>
        <input
          name="content"
          type="textarea"
